...
@SAIDEC PROC           ;INÍCIO DO PROCEDIMENTO PARA SAÍDA DE NÚMERO DECIMAL
    ...
    XOR CX,CX          
    MOV BX,10          ;CONFIGURA O DIVISOR PARA BASE 10 (DECIMAL)
    
@REP1:                 ;LABEL DE REPETIÇÃO PARA EXTRAIR DÍGITOS

    XOR DX,DX          
    DIV BX             ;DIVIDE AX POR 10, DX RECEBE O RESTO (DÍGITO MENOS SIGNIFICATIVO)
    PUSH DX            ;SALVA O RESTO (DÍGITO) NA PILHA
    INC CX             ;INCREMENTA O CONTADOR DE DÍGITOS

    OR AX,AX           ;VERIFICA SE AX É ZERO (PARADA DA DIVISÃO)
    JNE @REP1          ;SE NÃO FOR ZERO, CONTINUA DIVIDINDO
    MOV AH,2           ;EXIBE CARACTERE

@IMP_LOOP:             ;LABEL PARA IMPRIMIR OS DÍGITOS

    POP DX             
    OR DL,30H          ;CONVERTE O VALOR NUMÉRICO PARA ASCII
    INT 21H            ;IMPRIME O DÍGITO NA TELA
    LOOP @IMP_LOOP     ;REPETE ATÉ QUE CX SEJA ZERO (TODOS OS DÍGITOS IMPRESSOS)
...
@SAIDEC ENDP           ;FINALIZA O PROCEDIMENTO
